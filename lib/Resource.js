...
Resource.prototype.dependsOn = function(deps) {
  this.deps = this.dependsFilter(deps);
};
...
Resource.prototype.dependsFilter = function(deps) {
  if (Array.isArray(deps)) {
    var copiedDeps = [];
    var that = this;
    deps.forEach(function(currentValue, index, array) {
      copiedDeps[index] = that.dependsFilter(currentValue);
    });
    return copiedDeps;
  } else if (deps instanceof Resource) {
    return deps.key;
  } else {
    return deps;
  }
};
Resource.prototype.arrayFilter = function(refs, type) {
  var copiedRefs = [];
  var that = this;
  refs.forEach(function(currentValue, index, array) {
    copiedRefs[index] = that.simpleFilter(currentValue, type);
  });
  return copiedRefs;
};
