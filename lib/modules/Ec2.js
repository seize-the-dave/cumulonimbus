...
parseResourceId = function(prefix, name, parameter, type) {
  if (parameter instanceof type || parameter instanceof Parameter) {
...
Instance.prototype.setIamInstanceProfile = function(profile) {
  this.addProperty("IamInstanceProfile", profile.getRef());
};
...
Instance.prototype.setUserData = function(userData) {
  this.addProperty("UserData", userData);
};
...
SecurityGroup.validate = function(groupId) {
  return parseResourceId("sg", "Security Group", groupId, SecurityGroup);
};
SecurityGroup.validateIdArray = function(groupIds) {
  var copiedIds = [];
  groupIds.forEach(function(currentValue, index, array) {
    copiedIds[index] = parseSecurityGroupId(currentValue);
  });
  return copiedIds;
};
